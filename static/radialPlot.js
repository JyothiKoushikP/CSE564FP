...
const margin = {top: 100, right: 10, bottom: 0, left: 80},
...
svg.append("g")
  ...
  .append("g")
  .attr("text-anchor", d => {
    const angle = (x(d) + x.bandwidth() / 2 + Math.PI) % (2 * Math.PI);
    return angle < Math.PI ? "end" : "start";
  })
  .attr("transform", d => {
    const angle = (x(d) + x.bandwidth() / 2) * 180 / Math.PI - 90;
    const employment = newData.find(entry => entry["Area Name"] === d)["value"];
    const radius = y(employment) + 20; // Adjust radius based on value value
...
.on("mouseover", function(event, d) {
  ...
  tooltip.transition()
    .duration(200)
    .style("opacity", .9);
  tooltip.html(`<strong>${d["Area Name"]}</strong><br>Establishments: ${d.value}`)
